...
class DSAPusher(Star):
    ...
    聊天指令组 /DSA (共13条):
      ── 帮助 ──
      /DSA 帮助 (h/help)        — 显示所有命令帮助

      ── 查询 ──
      /DSA 任务 [n]              — 查询最近 n 个分析任务 (默认 5)
      /DSA 报告 [ID] (拉取)      — 拉取指定任务报告 (默认最新)
      /DSA 复盘                  — 推送最新的大盘复盘报告
      /DSA 自选股行情            — 查看自选股实时行情
      ...
      ── 自选管理 ──
      /DSA 我的自选 (自选列表)    — 查看当前自选股列表
      /DSA 增加自选 <代码>       — 添加股票到自选列表
      /DSA 删除自选 <代码>       — 从自选列表移除股票

      ── 推送控制 ──
      /DSA 开启推送 (开启通知)   — 开启自动推送通知
      /DSA 关闭推送 (关闭通知)   — 关闭自动推送通知
      /DSA 推送状态 (查询推送)   — 查询推送通知开关状态
    """
