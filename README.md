...
在聊天窗口直接发送以下指令（以 `大盘` 前缀区分，避免与其它插件冲突）：

| 指令 | 功能 | 示例 |
|------|------|------|
| **大盘任务** | 查看最近分析任务列表 | `大盘任务 10` (显示10条) |
| **大盘报告 [ID]** | 拉取指定任务报告全文 | `大盘报告` (最新) / `大盘报告 abc123` |
| **大盘复盘** | 推送最新大盘复盘报告 | `大盘复盘` |
| **自选行情** | 查看自选股实时行情 | `自选行情` |
| **历史分析 \<代码\>** | 查询个股历史分析报告 | `历史分析 301491` / `历史分析 汉桑科技` |
| **我的自选报告** | 批量推送所有自选股报告 | `我的自选报告` |
